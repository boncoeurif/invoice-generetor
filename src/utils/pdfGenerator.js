...
export const generateInvoicePDF = (invoice, t) => {
  try {
    ...
    doc.setTextColor(0, 0, 0);
    doc.setFont("helvetica", "bold");
    doc.text(invoice.clientName || 'Valued Customer', 120, 56);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.text(`${safeT('date', 'Date')}: ${invoice.date || new Date().toLocaleDateString()}`, 120, 62);
    doc.text(`Invoice ID: #${(invoice.id || Date.now()).toString().slice(-6).toUpperCase()}`, 120, 68);
    ...
    doc.save(`Invoice_${invoice.clientName.replace(/\s+/g, '_')}_${(invoice.id || Date.now()).toString().slice(-4)}.pdf`);
    return true;
  } catch (error) {
